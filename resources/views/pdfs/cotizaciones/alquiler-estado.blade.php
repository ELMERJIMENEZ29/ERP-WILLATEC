...
$formaPagoCalendario = in_array($formaPago, ['CRÉDITO A 5 DÍAS', 'CRÉDITO 15 DÍAS', 'CRÉDITO 30 DÍAS'], true) ? ' calendario' : '';
...
if (str_contains(strtolower($mime), 'png') && ! function_exists('imagecreatefrompng')) {
return null;
}
        ...
        <div class="items-wrap">
            <table class="items" cellpadding="0" cellspacing="0">
                ...
                <tbody>
                    ...
                    $precioUnitMensual = (float) $item->precio_venta;
                    $precioCantidadMensual = $precioUnitMensual * (float) $item->cantidad;
                    $destinosItem = $esMultidestino && $item->destinosEntrega->isNotEmpty()
                    ? $item->destinosEntrega
                    : collect([(object) [
                    'destino_entrega' => $item->destino_entrega ?: 'Lima Metropolitana',
                    ...
                    <tr class="{{ $loop->even ? 'even' : '' }}">
                        ...
                        <td>
                            ...
                            @endif
                            <span class="muted">Periodo: {{ $periodoMeses ?: '-' }} meses</span>
                        </td>
                        <td class="center">
                            @if($itemImageSrc)
                        ...
                        <td class="center">
                            ...
                            <div style="border-bottom: {{ $loop->last ? '0' : '1px solid #e5e7eb' }}; padding:2px 0;">
                                <span class="muted">
                                    {{ $destino->destino_entrega ?: 'Lima Metropolitana' }}
                                </span>
                                @if(!empty($destino->detalle_variante))
                                <br>
                                <span style="font-size:7.4px; color:#374151;">
                        ...
                        <td class="right">
                            @foreach($destinosItem as $destino)
                            <div style="border-bottom: {{ $loop->last ? '0' : '1px solid #e5e7eb' }}; padding:2px 0;">{{ $simbolo }} {{ number_format((float) $destino->precio_venta, 2) }}</div>
                            @endforeach
                        </td>
                        <td class="right">
                            @foreach($destinosItem as $destino)
                            <div style="border-bottom: {{ $loop->last ? '0' : '1px solid #e5e7eb' }}; padding:2px 0;">{{ $simbolo }} {{ number_format((float) $destino->precio_venta * (float) $destino->cantidad, 2) }}</div>
                            @endforeach
                        </td>
                        <td class="right strong">
                            @foreach($destinosItem as $destino)
                            <div style="border-bottom: {{ $loop->last ? '0' : '1px solid #e5e7eb' }}; padding:2px 0;">{{ $simbolo }} {{ number_format((float) $destino->subtotal, 2) }}</div>
                            @endforeach
                        </td>
                        <td class="center">
